...
const baseUrl = `https://gateway-staging.ncrcloud.com/catalog/v2`;
 ...
 * @returns {object} The response as { status: string, data: JSON }
 */
export async function getSiteCatalogItemDetails(siteId, query = null, page = 0) {
  console.log(siteId);
  if (query != null) {
    let data = await sendRequest(`${baseUrl}/item-details/search?shortDescriptionPattern=*${query}*&itemStatus=ACTIVE&sortField=ITEM_CODE&sortDirection=ASC&pageNumber=${page}`, 'GET', null, siteId);
    return data;
  } else {
    let data = await sendRequest(`${baseUrl}/item-details/search?itemStatus=ACTIVE&sortField=ITEM_CODE&sortDirection=ASC&pageNumber=${page}`, 'GET', null, siteId);
    return data;
  }
}
