 ...
 */
export default async function sendRequest(url, method, body = null, enterpriseUnit = null, returnHeaders = false, organization = null, jsonData = true) {
  let date = new Date();
  ...
  let headers = {
    ...
    sharedKey: process.env.REACT_APP_BSP_SHARED_KEY
    // content-md5?
  };
  ...
  if (organization !== null) {
    requestOptions.headers['nep-organization'] = organization;
  }
  ...
  /* else {
        ...
        var Infldata    = pako.inflate(binData);
        data            = String.fromCharCode.apply(null, new Uint16Array(Infldata))
    } */
  log.res = {
    status,
    data
