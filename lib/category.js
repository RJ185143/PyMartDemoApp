...
export async function getCategoryNodesForMenu() {
  let { categories, logs } = await getAllCategoryNodes(false, true);
...
export async function getAllCategoryNodes(flatten = false, includeDepartmentNodes = true) {
  ...
  if (response.status == 200) {
    ...
    const promises = parentCategories.map(async (category) => {
      const children = await getCategoriesByParentId(category.nodeId.nodeId);
      logs.push(children.log);
      let nodeCode = category.nodeId.nodeId;
      let childrenData = children.data.pageContent;
      let output = {};
      let outputArray = [];
      childrenData.forEach((child) => {
        output[child.nodeId.nodeId] = child;
        outputArray.push(child.nodeId.nodeId);
    ...
    parentCategories.forEach((category) => {
      category['children'] = parentCategoriesMap[category.nodeId.nodeId];
    ...
    let promiseOne = currentResult.map(async (root) => {
      ...
      children.array.forEach((child) => {
        copied.children[child]['children'] = parentCategoriesMap[root.nodeId.nodeId][child];
      });
  ...
  if (response.status == 200) {
    ...
    while (hierarchicalNodes.length != nodes.length) {
      if (hierarchicalNodes.length == 0) {
        ...
        nodes.find(function (element) {
          if (element.parentId && element.parentId.nodeId == lastNode.nodeId.nodeId) {
            hierarchicalNodes.push(element);
            return false;
          }
        });
